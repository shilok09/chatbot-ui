...
>(({ className, children, ...props }, ref) => (
  <DialogPortal>
    ...
    <DialogPrimitive.Content
      ref={ref}
      className={cn(
        "fixed left-[50%] top-[50%] z-50 grid w-full max-w-[90vw] md:max-w-[800px] translate-x-[-50%] translate-y-[-50%] gap-4 border bg-card p-0 shadow-xl duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 rounded-2xl",
        className
      )}
      {...props}
    >
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "default", size = "default", ...props }, ref) => {
    const variantClasses = {
      ...
      hero: "bg-gradient-primary hover:opacity-90 text-primary-foreground shadow-chat",
    };
    const sizeClasses = {
      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              "rounded-3xl border bg-card p-2 shadow-input transition-all duration-300",
  ...
  return (
    <>
      <PromptInput
        ...
        className={cn(
          "w-full bg-card border shadow-input transition-all duration-300 ease-in-out",
      ...
      >
            ...
            {files.map((file, index) => (
              <div key={index} className="relative group">
                {file.type.startsWith("image/") && filePreviews[file.name] && (
                  <div
                    className="w-16 h-16 rounded-xl overflow-hidden cursor-pointer transition-all duration-300 hover:scale-105"
                  ...
                  >
                    ...
                    <button
                      ...
                      className="absolute top-1 right-1 rounded-full bg-destructive/70 p-0.5 opacity-100 transition-opacity hover:bg-destructive"
                    >
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          ...
          >
            <Button
              variant={hasContent ? "hero" : "ghost"}
              size="icon"
              className={cn(
                "h-8 w-8 rounded-full transition-all duration-200",
                isRecording && "bg-transparent hover:bg-accent text-destructive hover:text-destructive",
                !hasContent && !isRecording && "text-muted-foreground hover:text-foreground"
              )}
              onClick={() => {
